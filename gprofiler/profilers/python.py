...
class PySpyProfiler(SpawningProcessProfilerBase):
    ...
    def _detect_corrupted_output(self, output_path: str) -> bool:
        ...
        try:
            ...
            total_content_lines = len([line for line in lines if line.strip() and not line.startswith("#")])
        ...
        with removed_path(local_output_path):
            try:
                ...
                if self._is_pyspy_crash(e.returncode, e.stderr):
                    logger.error(
                        f"py-spy crashed with signal {e.returncode} while profiling process {process.pid} ({comm}). "
                ...
                if self._is_embedded_python_error(e.stderr):
                    logger.info(
                        f"Process {process.pid} ({comm}) appears to embed Python but isn't a Python process - "
                        "skipping py-spy profiling"
                    )
                    return ProfileData(
                ...
                if self._is_process_exit_during_profiling(e.stderr, process):
                    logger.info(
                        f"Process {process.pid} ({comm}) exited during py-spy profiling - " "likely short-lived process"
                    )
                    return ProfileData(
                ...
                if self._is_missing_files_error(e.stderr, process):
                    logger.info(
                        f"Process {process.pid} ({comm}) has missing/deleted files during profiling - "
                        "likely temporary libraries or build artifacts"
                    )
                    return ProfileData(
            ...
            if self._detect_corrupted_output(local_output_path):
                logger.warning(
                    f"py-spy output for process {process.pid} ({comm}) appears corrupted or incomplete. "
    ...
    def _should_skip_process(self, process: Process) -> bool:
        ...
        try:
            ...
            if process_age < self._min_duration:
                logger.debug(
                    f"Skipping young process {process.pid} "
    ...
    def _is_embedded_python_process(self, process: Process) -> bool:
        ...
        try:
            ...
            if self._has_embedded_python_signature(process):
                logger.debug(
                    f"Process {process.pid} ({exe_basename}) appears to embed Python but isn't a Python process"
                )
                return True
...
@register_profiler(
    ...
    profiler_arguments=[
        ...
        ProfilerArgument(
            ...
            "When exceeded, PyPerf won't start but py-spy fallback is allowed. Default: %(default)s",
        ),
    ],
...
class PythonProfiler(ProfilerInterface):
    ...
    def _is_elf_symbol_error(self, stderr: str) -> bool:
        ...
        try:
            ...
            return (
                PythonEbpfProfiler._DELETED_LIBRARY_ERROR_PATTERN in stderr
                and PythonEbpfProfiler._DELETED_FILE_MARKER in stderr
            )
        except ImportError:
