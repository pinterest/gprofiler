...
gprofiler --max-processes-runtime-profiler 50 --skip-pyperf-profiler-above 50 --perf-use-cgroups --perf-max-cgroups 15
...
gprofiler --max-processes-runtime-profiler 25 --skip-pyperf-profiler-above 25 --perf-use-cgroups --perf-max-docker-containers 10 --perf-max-cgroups 0
# Result: PyPerf covers all Python + top 10 containers, <500MB memory

# Python-optimized: Maximize Python coverage, minimal perf
gprofiler --max-processes-runtime-profiler 40 --skip-pyperf-profiler-above 35 --skip-system-profilers-above 250
...
### Solution 4: PyPerf-Specific Threshold (`--skip-pyperf-profiler-above`) - OPTIMIZED eBPF CONTROL
...
gprofiler --skip-pyperf-profiler-above 50 --skip-system-profilers-above 300

# PyPerf-only threshold (optimal for Python-heavy workloads)
gprofiler --skip-pyperf-profiler-above 25 --max-processes-runtime-profiler 10

# Conservative approach for resource-constrained systems
gprofiler --skip-pyperf-profiler-above 15 --skip-system-profilers-above 200
```
...
**Files Modified:**
- `gprofiler/main.py`: Added `--skip-pyperf-profiler-above` CLI argument
